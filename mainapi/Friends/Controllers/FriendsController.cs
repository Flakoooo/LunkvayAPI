...
namespace LunkvayAPI.Friends.Controllers
{
    ...
    public class FriendsController(IFriendsService friendsService, ILogger<FriendsController> logger) : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetCurrentUserFriends([FromQuery] int page = 1, [FromQuery] int pageSize = 10)
        {
        ...
        [HttpGet("random")]
        public async Task<IActionResult> GetCurrentUserRandomFriends([FromQuery] int count = 6)
        {
            Guid userId = (Guid)HttpContext.Items["UserId"]!;
